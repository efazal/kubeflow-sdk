    ...
    while True:
        ...
        try:
            ...
            async with write_semaphore:  # Limit number of active disk writes
                await asyncio.to_thread(shutil.move, hidden_states_path, target_hidden_states_path)
                if validate_outputs:
            ...
            if fail_on_error:
                logger.exception("Fatal: sample %d aborted with --fail-on-error: %s", idx, e)
                logging.shutdown()
                os._exit(1)
...
async def generate_and_save_hidden_states(args, dataset):
    ...
    async with openai.AsyncOpenAI(base_url=args.endpoint, api_key="EMPTY", max_retries=0) as client:
    ...
    if skipped_indices:
        logger.warning(f"Skipped {len(skipped_indices)} samples due to errors: {skipped_indices}")
